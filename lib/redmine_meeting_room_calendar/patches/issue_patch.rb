          ...
          if (@issues[i].start_date.to_s < self.start_date.to_s) &&
            ...
            if c == 1
              for k in s do
                if @issues[i].d4_value == k then
                  err = User.find(k).to_s
                  errors.add err, "#{err} --> будет отсутствовать1!"
                  return false
                end
              end
            else
              if @issues[i].d4_value == s then
                err = User.find(s).to_s
                errors.add err, "#{err} --> будет отсутствовать2!"
                return false
              end
            end
          ...
          ##### внутри дня #####
            ...
            if c == 1
              for k in s do
                if @issues[i].d4_value == k then
                  err = User.find(k).to_s
                  errors.add err, "#{err} --> будет отсутствовать3!"
                  return false
                end
              end
            else
              if @issues[i].d4_value == s then
                err = User.find(s).to_s
                errors.add err, "#{err} --> будет отсутствовать4!"
                return false
              end
            end
            ...
            if (@issues[i].due_date.to_s==self.due_date.to_s) && (@issues[i].d3_value > self.custom_field_values[start_time].to_s) then
              if c == 1
                for k in s do
                  if @issues[i].d4_value == k then
                    err = User.find(k).to_s
                    errors.add err, "#{err} --> будет отсутствовать5!"
                    return false
                  end
                end
              else
                if @issues[i].d4_value == s then
                  err = User.find(s).to_s
                  errors.add err, "#{err} --> будет отсутствовать6!"
                  return false
                end
              end
              ...
              if c == 1
                for k in s do
                  if @issues[i].d4_value == k then
                    err = User.find(k).to_s
                    errors.add err, "#{err} --> будет отсутствовать7!"
                    return false
                  end
                end
              else
                if @issues[i].d4_value == s then
                  err = User.find(s).to_s
                  errors.add err, "#{err} --> будет отсутствовать8!"
                  return false
                end
              end
            end
