class MeetingRoomCalendarController < ApplicationController
  ...
  def initialize
    ...
    @project_id_ill = Setting['plugin_redmine_meeting_room_calendar']['project_ids_ill']
    @project_ids_ill = Setting['plugin_redmine_meeting_room_calendar']['project_ids_ill']

    @project_ids = @project_ids + [@project_ids_ill]
    ...
    if check_settings
     ...
     # @meeting_uchastniki = CustomField.find_by_id(@custom_field_id_uchastniki).possible_values
  ...
  def index
    ...
    @meeting_uchastniki = @project.assignable_users.collect { |user| [user.name, user.id] }
  ...
  def create
    ...
    @project_id = @project.id
if @project_ids_ill.include?(@project_id.to_s)
